...
module.exports = function(RED) {
    ...
    function RayServe(config) {
        ...
        const queueStatus = () => {
            ...
            const current = node.statusCount <= 10 ? '|'.repeat(node.statusCount) : node.statusCount;
        ...
        const processNext = () => {
            ...
            axios({ method: 'get', url: node.url, data: JSON.stringify(msg.payload) })
                .then(r => {
                    send({ ...msg, payload: r.data }); 
                    updateStatusCount(-1);
                    done();
                    processNext();
                })
                .catch(error => { 
                    node.error({ ...msg, error, statusCode: error.response?.status })
                ...
                .then(() => {
                    ...
                    let ws = api.watchStatus(({ applications }) => {
                        nodeList.forEach(n => {
                            ...
                            n?.trackStatus(STATES?.[key] || {...STATES.UNKNOWN, text: key});
