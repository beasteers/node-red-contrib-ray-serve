...
module.exports = function(RED) {
    ...
    function RayServe(config) {
        ...
        node.trackStatus = (status) => {
            ...
            const isRunning = node.lastStatus.msg == STATES.RUNNING.text;
            if(node.statusCount && isRunning) return;
            node.status(status);
        }
        ...
        const updateStatusCount = (increment, error=false) => {
            ...
            const isRunning = node.lastStatus.msg == STATES.RUNNING.text;
            const msg = error || (isRunning ? "" : node.lastStatus?.msg);
            node.status({
                ...(error ? STATES.ERROR_QUEUED : node.lastStatus), 
                text: `${node.statusCount < 10 ? '|'.repeat(node.statusCount) : node.statusCount} ${msg||""}`,
                msg
            });
        }
            ...
            api.waitForAPI()
                ...
                .then(() => {
                    ...
                    let ws = api.watchStatus(({ applications }) => {
                        nodeList.forEach(n => {
                            // console.log(n.name, applications[n.name]?.status, n.statusCount)
                            let status = STATES?.[applications[n.name]?.status] || {...STATES.UNKNOWN, text: applications[n.name]?.status};
                            n?.trackStatus(status);
                        })
                    })
