                ...
                return this.getApplications()
                    .then(current => {
                        ...
                        if(isDeepEqual(applications, currentApplications)) {
                            console.log("Applications are the same, skipping upload");
                            return { status: 204, data: '' };
                        }
                        console.log("Applications are different, uploading");
                        return this.uploadServeConfig({ ...config, applications });
                    });
            })
            .then(d => {
                if(d.status !== 204) console.log('Upload completed!', d.status, d.data)
                return d;
            })
            .catch(e => {
                console.error(e);
